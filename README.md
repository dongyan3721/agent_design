...
make docker-up# Start frontend (separate command)
...
Each generated project includes a CLI tool named `med_agent`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run med_agent server run --reload     # Start dev server
uv run med_agent server routes           # Show all routes# User commands
uv run med_agent user create-admin       # Create admin user
uv run med_agent user create             # Create regular user
uv run med_agent user list               # List all users```

Or use Makefile shortcuts from the project root:

```bash
make help          # Show all available commands
make run           # Start dev servermake create-admin  # Create admin user```
...
# Server
uv run med_agent server run --reload
uv run med_agent server routes

# Database (Alembic wrapper)
uv run med_agent db init
uv run med_agent db migrate -m "Add users"
uv run med_agent db upgrade

# Users
uv run med_agent user create-admin       # Create admin (interactive)
uv run med_agent user create             # Create user (interactive)
uv run med_agent user list               # List all users
```
...
```bash
uv run med_agent cmd seed --count 100
uv run med_agent cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend med_agent server run --reload` |
| Start prod server | `uv run --directory backend med_agent server run --host 0.0.0.0 --port 8000` |
| Show routes | `uv run --directory backend med_agent server routes` |
...
| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend med_agent user create-admin` |
| Create user | `uv run --directory backend med_agent user create` |
| List users | `uv run --directory backend med_agent user list` |
### Cleanup
